/*
 * This file is part of busybox (www.busybox.net)
 */
...
#define ALIGN1
#include <stdio.h>
#define auto_string(a) (a)
static char *xasprintf(const char *fmt,unsigned long long val,
		       unsigned frac, int u) {
  static char buf[32];
  snprintf(buf,sizeof(buf),fmt,val,frac,u);
  buf[sizeof(buf)-1] = '\0';//Make sure it is NUL terminated
  return buf;
}
